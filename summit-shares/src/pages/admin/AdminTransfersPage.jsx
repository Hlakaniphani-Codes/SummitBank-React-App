                      ...
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                        {t.status === 'pending' && (
                          <>
                            ...
                            <button className="admin-btn admin-btn-secondary admin-btn-xs" onClick={() => handleNotify(t.id)}>Notify</button>
                          </>
                        )}
                        {t.status === 'approved' && (
                          <>
      ...
      <div className={`admin-modal-overlay ${confirmModal.open ? 'active' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) setConfirmModal({ open: false, action: '', transferId: null, transferLabel: '' }); }}>
        <div className="admin-modal" style={{ maxWidth: 400 }}>
          ...
          <div className="modal-actions">
            <button className="admin-btn admin-btn-secondary" onClick={() => setConfirmModal({ open: false, action: '', transferId: null, transferLabel: '' })}>Cancel</button>
            <button className={`admin-btn ${['block','reject'].includes(confirmModal.action) ? 'admin-btn-danger' : 'admin-btn-success'}`} onClick={handleConfirmAction}>
              <i className={`fas ${confirmModal.action === 'approve' ? 'fa-check' : confirmModal.action === 'hold' ? 'fa-pause' : confirmModal.action === 'block' ? 'fa-ban' : confirmModal.action === 'unblock' ? 'fa-unlock' : confirmModal.action === 'remove-hold' ? 'fa-play' : confirmModal.action === 'mark-sent' ? 'fa-paper-plane' : 'fa-check-circle'}`}></i>
      ...
      <div className={`admin-modal-overlay ${reasonModal.open ? 'active' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) setReasonModal({ open: false, transferId: null }); }}>
        <div className="admin-modal" style={{ maxWidth: 450 }}>
          ...
          <form onSubmit={handleReject}>
            ...
            <div className="modal-actions">
              <button type="button" className="admin-btn admin-btn-secondary" onClick={() => setReasonModal({ open: false, transferId: null })}>Cancel</button>
              <button type="submit" className="admin-btn admin-btn-danger"><i className="fas fa-times"></i> Reject</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div className={`admin-modal-overlay ${failedModal.open ? 'active' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) setFailedModal({ open: false, transferId: null }); }}>
        <div className="admin-modal" style={{ maxWidth: 450 }}>
          ...
          <form onSubmit={handleMarkFailed}>
            ...
            <div className="modal-actions">
              <button type="button" className="admin-btn admin-btn-secondary" onClick={() => setFailedModal({ open: false, transferId: null })}>Cancel</button>
              <button type="submit" className="admin-btn admin-btn-danger"><i className="fas fa-exclamation-triangle"></i> Mark Failed</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div className={`admin-modal-overlay ${errorModal.open ? 'active' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) setErrorModal({ open: false, transferId: null }); }}>
        <div className="admin-modal">
          ...
          <form onSubmit={handleSetError}>
            ...
            <div className="modal-actions">
              <button type="button" className="admin-btn admin-btn-secondary" onClick={() => setErrorModal({ open: false, transferId: null })}>Cancel</button>
              <button type="submit" className="admin-btn admin-btn-danger"><i className="fas fa-exclamation-triangle"></i> Set Error</button>
            </div>
          </form>
        </div>
      </div>
